...
//        test6(context);
    ...
    void test7(AnnotationConfigApplicationContext context) throws Exception {
    ...
    private static void test6(AnnotationConfigApplicationContext context) throws Exception {
        ...
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            ...
            if (!container.isRequestHandled()) {
                ...
                for (String name : response.getHeaderNames()) {
                    System.out.println("响应头：" + name + "=" + response.getHeader(name));
                }
                System.out.println("响应体：" + new String(response.getContentAsByteArray(), StandardCharsets.UTF_8));
            }
        }
    }
    ...
    // 方法上没有@ModelAttribute，ServletModelAttributeMethodProcessor来处理
    ...
    // 方法上的@ModelAttribute，ServletModelAttributeMethodProcessor来处理
    // 方法的返回值作为模型数据，添加到 ModelAndViewContainer
    ...
    private static void test1(AnnotationConfigApplicationContext context) throws Exception {
        Method method = Controller.class.getMethod("test1");  // 返回值类型是 ModelAndView
        Controller controller = new Controller();
        Object returnValue = method.invoke(controller); // 获取返回值
        ...
        if (composite.supportsReturnType(methodHandle.getReturnType())) { // 检查是否支持此类型的返回值
            composite.handleReturnValue(returnValue, methodHandle.getReturnType(), container, webRequest);
            // model作为模型数据存入 ModelAndViewContainer
            System.out.println(container.getModel());
            // 视图名在 ModelAndView 的构造方法中指定
            System.out.println(container.getViewName());
            renderView(context, container, webRequest); // 渲染视图
        }
    }
    ...
    public static HandlerMethodReturnValueHandlerComposite getReturnValueHandler() {
        ...
        composite.addHandler(new ModelAndViewMethodReturnValueHandler());  // 处理ModelAndView
    ...
    static class Controller {
        ...
        @ModelAttribute
//        @RequestMapping("/test3")  // 找不到视图名时，把路径的名字(test3)当做是视图名
        public User test3() {
            log.debug("test3()");
        ...
        public HttpEntity<User> test5() {
            log.debug("test5()");
            return new HttpEntity<>(new User("赵六", 40)); // java对象转换成响应体
        }
        ...
        // ResponseBody 代表响应体
        @ResponseBody  // 返回的内容，作为响应体,自动设置响应头 ： Content-Type=application/json
        public User test7() {
            log.debug("test7()");
            return new User("钱七", 50);  // java对象转换成响应体
        }
    }
