...
//        test3();
    ...
    // 多个 MessageConverter 协作，那么用哪个 MessageConverter 来处理返回值呢
    private static void test4() throws IOException, HttpMediaTypeNotAcceptableException, NoSuchMethodException {
        ...
        response.setContentType("application/json"); // 会生成media-type
    ...
    private static void test3() throws IOException {
        MockHttpInputMessage message = new MockHttpInputMessage(new String("                    {\n" +
                                                                           "                        \"name\":\"李四\",\n" +
                                                                           "                        \"age\":20 \n" +
                                                                           "                    }").getBytes());
    ...
    public static void test1() throws IOException {
        MockHttpOutputMessage message = new MockHttpOutputMessage();
        MappingJackson2HttpMessageConverter converter = new MappingJackson2HttpMessageConverter();  // HttpMessageConverter 对象
        // 判断能不能转
        if (converter.canWrite(User.class, MediaType.APPLICATION_JSON)) {
            // 对象 -> json，转换结果写入 message
